...
export default function PlacementTestPage() {
  ...
  useEffect(() => {
    if (!activeTrack || (!results[activeTrack.trackId] && !activeTrack.latestAttempt)) return

    const nextPendingIndex = tracks.findIndex(
      (track) => !results[track.trackId] && !track.latestAttempt,
    )

    if (nextPendingIndex >= 0 && nextPendingIndex !== activeTrackIndex) {
      setActiveTrackIndex(nextPendingIndex)
    }
  }, [activeTrack, activeTrackIndex, results, tracks])
