            ...
            <div className="max-w-5xl mx-auto px-4 py-8">
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-8">
                        {page > 1 && (
                            <Link href={`/community/questions?filter=${filter}&page=${page - 1}${tag ? `&tag=${tag}` : ""}${track ? `&track=${track}` : ""}`} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-bold hover:border-emerald-400 transition-colors">
                                السابق
                            </Link>
                        )}
