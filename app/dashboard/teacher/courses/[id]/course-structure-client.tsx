...
import {
  ...
  Settings, Award, RefreshCw, Calendar, Layers, Upload, Loader2
} from "lucide-react"
...
export default function CourseStructureClient({ course: initialCourse }: { course: any }) {
  ...
  // Track hierarchy state — initialized from the course's existing track
  const [tracks, setTracks] = useState<any[]>([])
  const [selectedRootId, setSelectedRootId] = useState<string>(() => {
    if (!course.track) return ""
    // If track has a grandparent (depth 3), root = grandparent; else if has parent, root = parent; else root = itself
    if (course.track.parent?.parentId) return course.track.parent.parentId
    if (course.track.parentId) return course.track.parentId
    return course.track.id
  })
  ...
  const [selectedLeafId, setSelectedLeafId] = useState<string>(() => {
    if (!course.track) return ""
    // Only a leaf if it has a parent
    if (course.track.parentId) return course.track.id
    return ""
  })

  useEffect(() => {
    getLearningTracks().then(res => {
      if (res.success && res.tracks) setTracks(res.tracks)
    })
  }, [])
  ...
  const handleMetadataChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setMetadata(prev => ({ ...prev, [name]: name === "priceInCoins" || name === "price" ? Number(value) : value }))
  }
  ...
  const handleSaveMetadata = async () => {
    setLoading(true)
    setStatusMsg(null)
    const finalTrackId = selectedLeafId || selectedSubId || selectedRootId || undefined
    const res = await updateCourseDetails(course.id, { ...metadata, trackId: finalTrackId } as any)
    if (res.success) {
                ...
                >
                  ...
                  {!isCollapsed && (
                    <div className="p-4 space-y-4 bg-white dark:bg-slate-950/20">
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-2 text-[9px] text-gray-400 mt-0.5">
                                    ...
                                    <span className="font-extrabold text-indigo-650 dark:text-indigo-400">
                                      {les.type === "VIDEO" ? "فيديو شرح" : les.type === "TEXT" ? "شرح نصي" : les.type === "BOTH" ? "فيديو وشرح نصي" : "ملف مرفق"}
                                    </span>
                                  </div>
                                </div>
                              </div>
                      ...
                      {activeAddForm && activeAddForm.moduleId === mod.id && (
                              ...
                              <div className="flex flex-wrap gap-4">
                                ...
                                <label className="text-[11px] text-gray-400 font-bold flex items-center gap-1">
                                  <input
                                    type="radio"
                                    name="lessonType"
                                    checked={lessonForm.type === "BOTH"}
                                    onChange={() => setLessonForm({ ...lessonForm, type: "BOTH" })}
                                  />
                                  كلاهما معاً (فيديو ونصوص)
                                </label>
                                <label className="text-[11px] text-gray-400 font-bold flex items-center gap-1">
                                  <input
                                    type="radio"
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-3xl border border-gray-200/60 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 space-y-5 shadow-sm text-right">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">السعر (عملات CC)</label>
              <input
                type="number"
                name="priceInCoins"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">السعر (دولار $)</label>
              <input
                type="number"
                name="price"
                step="0.01"
            ...
            {/* ── Track Hierarchy Selectors ── */}
            <div className="space-y-2 rounded-xl border border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/50 dark:bg-indigo-950/20 p-3">
              <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                <Layers className="w-3 h-3" />
                المسار التعليمي
              </p>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">القسم الرئيسي</label>
                <select
                  value={selectedRootId}
                  onChange={(e) => { setSelectedRootId(e.target.value); setSelectedSubId(""); setSelectedLeafId("") }}
                  className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl text-xs outline-none text-gray-700 dark:text-slate-300 focus:ring-1 focus:ring-indigo-500"
                >
              ...
              {selectedRootId && tracks.find((t: any) => t.id === selectedRootId)?.children?.length > 0 && (
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">المسار التعليمي الفرعي</label>
                  <select
                    value={selectedSubId}
                  ...
                  >
                    ...
                    {tracks.find((t: any) => t.id === selectedRootId)?.children.map((sub: any) => (
              ...
              {selectedSubId && tracks.find((t: any) => t.id === selectedRootId)?.children?.find((sub: any) => sub.id === selectedSubId)?.children?.length > 0 && (
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">مستوى المسار</label>
                  <select
                  ...
                  >
                    ...
                    {tracks.find((t: any) => t.id === selectedRootId)?.children?.find((sub: any) => sub.id === selectedSubId)?.children?.map((leaf: any) => (
              ...
              {(selectedRootId || selectedSubId || selectedLeafId) && (
                <p className="text-[9px] text-indigo-500 dark:text-indigo-400">
                  المسار المحدد: {[
                    tracks.find((t: any) => t.id === selectedRootId)?.name,
                    tracks.find((t: any) => t.id === selectedRootId)?.children?.find((s: any) => s.id === selectedSubId)?.name,
                    tracks.find((t: any) => t.id === selectedRootId)?.children?.find((s: any) => s.id === selectedSubId)?.children?.find((l: any) => l.id === selectedLeafId)?.name,
                  ].filter(Boolean).join(" ← ")}
                </p>
              )}
            </div>
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-500 dark:text-slate-400">صورة غلاف الدورة</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-850 rounded-xl text-xs outline-none focus:ring-1 focus:ring-indigo-500"
                  dir="ltr"
                />
                <input
                  type="file"
                  ...
                  onChange={async (e) => {
                    ...
                    if (url) {
                      ...
                      setStatusMsg({ type: "error", text: "فشل رفع صورة الغلاف، حاول مرة أخرى." })
                    }
                ...
                <label
                  ...
                  className="px-3 py-2 bg-indigo-50 text-indigo-650 dark:bg-indigo-950/40 dark:text-indigo-400 hover:bg-indigo-100 rounded-xl text-[10px] font-bold flex items-center gap-1.5 cursor-pointer flex-shrink-0 transition-colors border border-indigo-100 dark:border-indigo-900/40"
                >
                  {uploadingCover ? (
